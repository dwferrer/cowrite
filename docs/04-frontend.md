...
- **Prompt region breakdown** binds to the run `meta` event's `contextSnapshot`
  ...
  The schema keeps `items` **flat** (no per-region key), so the viewer renders one nested
  "context items" list under the region breakdown rather than expanding items per region;
  expanding the `message` events shows the raw prompt text — the user can see exactly what
  the model saw, one click deep.
...
// Ports (e2e/util.ts constants): 2698 for the server (distinct from the 2697 default so a
// running dev server never collides), 2700 for the in-process mock LLM's control routes;
// 2699 is reserved for the restart spec's own server. The server boots @cowrite/mock-llm
// IN-PROCESS under COWRITE_MOCK_LLM=1; MOCK_LLM_PORT pins it so specs can script it.
const dataDir = mkdtempSync(join(tmpdir(), "cowrite-e2e-data-"));
export default defineConfig({
  webServer: {
    ...
    url: "http://127.0.0.1:2698/api/health",
  },
...
dir per test. The mock LLM is scripted per test via its control routes on the mock's own port
(`POST http://127.0.0.1:2700/__mock/scenario` with canned `LlmStep[]`, `POST /__mock/reset`,
`GET /__mock/state` — step types imported from `@cowrite/mock-llm`).
